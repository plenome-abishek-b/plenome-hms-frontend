  ...
  const handleFormSubmit = async () => {
    try {
      if (formData?.name === "" || formData?.name === undefined) {
        ...
        formData?.finding_category_id === "" ||
        formData?.finding_category_id === undefined
      ) {
      ...
      >
        ...
        <DialogContent className="mt-4 ms-2">
          <Container className="p-2">
            <Row>
              <label>
                Finding<span style={{ color: "red" }}>*</span>
              </label>
              <br />
              <input
                onChange={handleChange}
                name="name"
                placeholder={validate?.name ? "Enter finding name" : ""}
                value={formData?.name}
                type="text"
                style={{
                  height: "30px",
                  borderColor: validate?.name ? "red" : "inherit",
            ...
            <Row>
              ...
              <br />
              <select
                onClick={getSetupFindingCategory}
                name="finding_category_id"
                value={formData?.finding_category_id}
                onChange={handleChange}
                style={{
                  height: "30px",
                  borderColor: validate?.finding_category_id
                    ? "red"
                    : "inherit",
                    border: '1px solid grey',
                  borderRadius: '5px'
            ...
            <Row>
              ...
              <textarea
                name="description"
                placeholder={validate?.description ? "Enter description" : ""}
                value={formData?.description}
                onChange={handleChange}
                style={{
                  height: "50px",
                  borderColor: validate?.description ? "red" : "inherit",
                  border: '1px solid grey',
                  borderRadius: '5px'
                }}
              ></textarea>
