...
const Checkmobileotp = props => {
  ...
  function handleFormSubmit() {
    api.checkMobileOtp({ mobile, txnId }).then(resp => {
      console.log(resp);
      const { data } = resp;
      ...
      if (data.mobileLinked === true) {
        toast.success("Mobile Number is linked already");
        api.creationOfAbhaId({ txnId }).then(resp => {
          console.log(resp);
          const responseData = resp.data;
          console.log(responseData, 'response here');
          setResData(responseData);
          localStorage.setItem('responseData', JSON.stringify(responseData)); // Store in localStorage
          history.push(`/account/abha/profile/${responseData}`);
        });
      } else {
        history.push(`/account/abha/verifymobileotp/${data.txnId}`);
        console.log(txnID, 'txnidddd');
      }
    }).catch(error => {
      console.error(error);
      toast.error("Error occurred while checking mobile OTP.");
    });
  }
                ...
                <CardBody>
                  <div className="bg-primary">
                    <Row>
                      <Col xs={7}>
                        <div className="p-5">
                          <h2 className="text-white">Mobile OTP Verification</h2>
                        </div>
                      </Col>
...
export default withRouter(Checkmobileotp)
