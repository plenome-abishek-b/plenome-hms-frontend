...
const PostalDispatch = props => {
 ...
 const Receivers =async ()=>{
   const response = await api.getFrontofficeDispatch()
   const {data} = response
   setTableData(data)
    ...
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          {/* Render Breadcrumb */}
          <Breadcrumbs
            title={props.t("Postal Dispatch list")}
            breadcrumbItem={props.t("Postal Dispatch list")}
          />
          <div style={{display: 'flex', justifyContent: 'flex-end', alignItems: 'flex-end'}}>
            <button className="btn btn-success" style={{marginRight: '10px'}} onClick={handleClickOpen}>Add Dispatch</button>
          </div>
          <DispatchDialog
            open={open}
...
export default withTranslation()(PostalDispatch);